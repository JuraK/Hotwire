...
file_paths=file_paths[124:155]
ax=ax[124:155]
vir=vir[124:155]
natoceni=natoceni[124:155]
#------------------------------------------------------------------------
...
for v in vir:
  ...
  for xi,ax_pozice in enumerate(ax):
    ...
    text='x/D='+ax_pozice
    yticks = [0,1,2,3,4,5]
    axes[xi].text(0.13, 5.1, text,fontsize=8,bbox={'facecolor':'white', 'alpha':0.5, 'pad':10})
  ...
  for xi,ax_pozice in enumerate(ax):
    ...
    for nat in natoceni:
      ...
      if len(n) > 0:
        ...
        r=(150-cta[n].x)
        Angle=cta[n].phi
        axes[xi].plot(r,Angle,label=nat, marker='+')
        
    if xi==4:
        axes[xi].set_xlabel('r [mm]',fontsize=12)
        axes[xi].set_ylim([-45,45])
    axes[xi].legend(loc=2)
    axes[xi].grid()
    axes[xi].set_ylabel('Angle [deg]',fontsize=8)
    
    axes[xi].set_xlim([0,150.3])
    text='x/D='+ax_pozice
    yticks = [-45,-40,-20,0,20, 40]
    axes[xi].text(40, 30, text,fontsize=8,bbox={'facecolor':'white', 'alpha':0.5, 'pad':10})
  fig.subplots_adjust(hspace=0)
  plt.setp([a.set_yticklabels(yticks) for a in fig.axes[:-2]], visible=True)
  plt.rcParams.update({'font.size': 8})
  ...
  for xi,ax_pozice in enumerate(ax):
    ...
    for nat in natoceni:
      ...
      if len(n) > 0:
        ...
        axes[xi].plot(r,Ek,label=nat, marker='+')
        
    if xi==4:
        axes[xi].set_xlabel('r [mm]',fontsize=12)
        axes[xi].set_ylim([0,2.5])
    axes[xi].legend(loc=2)
    axes[xi].grid()
    axes[xi].set_ylabel('Ek [J]',fontsize=8)
    
    axes[xi].set_xlim([0,150.3])
    text='x/D='+ax_pozice
    yticks = [0,0.5,1,1.5,2,]
    axes[xi].text(40, 2.1, text,fontsize=8,bbox={'facecolor':'white', 'alpha':0.5, 'pad':10})
  fig.subplots_adjust(hspace=0)
  plt.setp([a.set_yticklabels(yticks) for a in fig.axes[:-2]], visible=True)
  plt.rcParams.update({'font.size': 8})
  print 'Writing file: '+file_paths[nazev_ind][:-21]+'Ek.png' #('/').join([v,ax_pozice,'U_mag.png'])
  
  plt.savefig(file_paths[nazev_ind][:-21]+'Ek.png',dpi=500,bbox_inches='tight')
  plt.close() 
...
for v in vir:
  ...
  plt.grid()
  plt.rcParams.update({'font.size': 12})
  plt.savefig(file_paths[nazev_ind][:26]+'Ek_x.png',dpi=500,bbox_inches='tight')
  print 'Writing file: '+file_paths[nazev_ind][:26]+'Ek_x.png' #('/').join([v,ax_pozice,'U_mag_ax.png'])
  plt.close()      
## ----------------------------------------------------------------
